...
<body>
	...
	<section class="section_entire">
		<article class="search_area_entire">
			<form action="menu1.php" method="get"><!--  나중에 post로 변경 -->
...
if(!isset($bookname_s)){
	while(list($booklistnum, $name, $writer, $publisher)=mysqli_fetch_row($result)){
		echo "<script>";
		echo "$('.booklist_entire table').append('<tr><td>$booklistnum</td><td>$name&nbsp;\[$publisher\]</td><td>$writer</td><td id=\'borrow_check$booklistnum\'></td></tr>')";
		echo "</script>";
	 ...
	 while($check_borrow_row = mysqli_fetch_assoc($check_borrow_result)){
      if($booklistnum == $check_borrow_row['book_num']){
         $borrow_check=1;
         echo "<script>";
         echo "$('.booklist_table').find('#borrow_check$booklistnum').html('대여중')";
         echo "</script>";
         break;
      }else{
         $borrow_check=0;
